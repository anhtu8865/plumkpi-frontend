...
const EditDashboardButton = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values, { resetForm }) => {
      // console.log(values)
      api
        .put(`dashboards/dashboard?dashboard_id=${selectedDashboard}`, {
        ...
        .then(() => {
          dispatch(
            createAlert({
              message: 'Cập nhật bảng điều khiển mới thành công.',
              type: 'success',
            }),
          )
          dispatch(
  ...
  return (
    <>
      <Tooltip title="Chỉnh sửa dashboard">
        <IconButton id="edit" color="primary" size="small" onClick={() => setModalVisible(true)}>
      ...
      <form onSubmit={formik.handleSubmit}>
        ...
        >
          <CModalHeader>
            <CModalTitle>Sửa bảng điều khiển</CModalTitle>
          </CModalHeader>
          <CModalBody>
            <CRow className="mt-2">
              <CCol>
